...
import { Search, Loader2, AlertCircle, X, Eye, WifiOff, CheckCircle } from 'lucide-react'

// ---------------------------------------------------------------------------
// Detail Modal (Claimed-specific)
// ---------------------------------------------------------------------------
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-lg font-semibold text-white">Claimed Item #{item.id} Detail</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <Section title="Timestamps">
            <Field label="Date Claimed" value={item.updated_at ? new Date(item.updated_at).toLocaleString() : '—'} />
...
function ClaimedItems() {
  ...
  const { searched } = useItemFilter(items, 'claimed', searchQuery)
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-1">Items that have been successfully claimed</p>
      </div>
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by item name, claimant, surrenderer, department, or ID..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 bg-gray-900 border border-gray-800 rounded-lg text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
          />
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        ...
        {loading && items.length === 0 ? (
          ...
          <div className="p-12 flex flex-col items-center justify-center text-gray-500">
            ...
            <p className="text-sm">
              {searchQuery ? 'No claimed items match your search' : 'No claimed items'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="text-left px-6 py-3 text-gray-400 font-medium">Date Claimed</th>
