...
describe('Database Module', () => {
    ...
    it('creates the persons table with correct columns', async () => {
        const columns = await db.all("SELECT name, type FROM pragma_table_info('persons') ORDER BY cid");
    ...
    it('creates the items table with correct columns', async () => {
        const columns = await db.all("SELECT name, type FROM pragma_table_info('items') ORDER BY cid");
        const colMap = Object.fromEntries(columns.map(c => [c.name, c.type]));
        expect(colMap.id).toBeDefined();
    ...
    it('rejects invalid status values via CHECK constraint', async () => {
        await expect(db.run(`INSERT INTO items (id, item_name, description, category, department_origin, status)
         VALUES (?, ?, ?, ?, ?, ?)`, 'test-id-check', 'Test Item', null, null, 'TestDept', 'deleted')).rejects.toThrow();
    });
    it('enforces foreign key constraints on surrendered_by', async () => {
        // Insert with a non-existent person reference — must be rejected
        await expect(db.run(`INSERT INTO items (id, item_name, description, category, department_origin, status, surrendered_by)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, 'test-fk-1', 'Test FK Item', null, null, 'TestDept', 'found', 'nonexistent-person-id')).rejects.toThrow();
    });
});
